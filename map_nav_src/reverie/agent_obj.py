...
from utils.utils_tta import get_logger

from utils.misc_tta import (
    random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts
)

import models.FSTTA as fstta

from models.FSTTA import principal_grad
...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def rollout(self, TTA_lr_r, sar_margin_e0_r, output_dir_r, train_ml=None, train_rl=False, reset=True):
        ...
        # condiguration for learninig rate
        lr_fast = 6e-4
        lr_slow = 1e-3
        N = 4
        q = 0.1

        logger_tta = get_logger(name="project", output_directory=output_dir_r, log_name='info_tta.txt', debug=False)
        # net = self.models.cuda()
        # global tta_inputs
        #tta_inputs = nav_inputs

        # net = VLNBert() 
        # net.load_state_dict(torch.load(PATH), strict=False) 
        #"Slow stage model parameters"
        net = self.vln_bert.cuda() 

        net = fstta.configure_model(net)
        
        params, param_names = fstta.collect_params(net)
        optimizer = torch.optim.AdamW(params, lr_fast, betas=(0.9, 0.99)) 

        # ---- FAST: init once per rollout ----
        adapt_model = fstta.FAST(
            net, optimizer,
            t=0,               # from step 0
            lr_fast=lr_fast,
            steps=1,
        ...
        # ----- SLOW initialization (persistent across samples) -----
        # Initialize these attributes only once
        if not hasattr(self, "slow_initialized"):
            self.slow_initialized = True
            self.slow_id = 0

            # Create a slow model as a historical reference
            self.net_slow = copy.deepcopy(net)
            slow_params, slow_param_names = fstta.collect_params(self.net_slow)
            self.optimizer_slow = torch.optim.AdamW(
                slow_params, lr_slow, betas=(0.9, 0.99)
            )

            # Parameter trajectory stack: first state is Θ̃_{l,0}
            self.params_stack_slow = {
                key: {"value": []} for key, _ in self.net_slow.named_parameters()
            }
            for (key, _), p in zip(self.params_stack_slow.items(), self.net_slow.parameters()):
                self.params_stack_slow[key]["value"].append(
                    copy.deepcopy(p).unsqueeze(0)
                )

            # Initialize reference direction h_l as zero ParamDict
        ...
        for t in range(self.args.max_action_len):
            ...
            # adapt_model = fstta.FAST(net, optimizer, t, lr_fast)
            #%start1 = time.time()
            nav_outs, loss = adapt_model(nav_inputs)
            ...
            #nav_probs = torch.softmax(nav_outs['fused_logits'], 1)
            #loss_a = softmax_entropy(nav_probs).mean(0)
        ...
        # When a test sample ends
        # --------------------------------------------------------
        #  SLOW update: invoked once per test sample (episode)
        # --------------------------------------------------------
        # Count how many samples have been processed since last slow update
        self.slow_id += 1

        # (1) Append current fast-updated parameters to the trajectory stack
        for (key, _), p in zip(self.params_stack_slow.items(), net.parameters()):
            self.params_stack_slow[key]["value"].append(
                copy.deepcopy(p).unsqueeze(0)
            )

        # (2) Compute parameter deviation between slow model and current model
        origin_weights = ParamDict(self.net_slow.named_parameters())
        update_weights = ParamDict(net.named_parameters())
        diff_weights = origin_weights - update_weights

        # (3) Update reference direction h_l (Eq. (8)) with exponential weights
        denom = sum(pow(q, N - n) for n in range(1, N + 1))  # normalization
        weight = pow(q, N - self.slow_id)
        self.ref_diff += (weight / denom) * diff_weights

        # (4) When N samples are collected, perform one SLOW update
        if self.slow_id == N:
            slow_adapter = fstta.SLOW(self.net_slow, net, self.optimizer_slow, N)
            slow_adapter(self.params_stack_slow, self.ref_diff)

            # Reset counters for the next window of N samples
            self.slow_id = 0

            # Rebuild trajectory stack: first state is the updated slow model Θ̃_{l+1,0}
            self.params_stack_slow = {
                key: {"value": []} for key, _ in self.net_slow.named_parameters()
            }
            for (key, _), p in zip(self.params_stack_slow.items(), self.net_slow.parameters()):
            ...
            # Reset reference direction h_l to zero
            origin_weights = ParamDict(self.net_slow.named_parameters())
            self.ref_diff = origin_weights - origin_weights
        '''
        id += 1
        # Slow Update by Principal Parameters Adjustment
        # Our Slow Update performs every N samples.
        # Record the model parameters after fast update during each test sample.
        params_stack_slow = {key: {'value':[]} for key, _ in net.named_parameters}
        for (key, _), env_value in zip(params_stack_slow.items(), net.parameters()):
            params_stack_slow[key]['value'].append(copy.deepcopy(env_value).unsqueeze(0))
        update_weights = ParamDict(net.named_parameters()) 
        origin_weights = ParamDict(net_slow.named_parameters())
        # Calculate the reference direction for slow update
        diff_weights = origin_weights - update_weights
        ref_diff += pow(q, N-id) * diff_weights / (sum(pow(q, N-n) for n in range(N)))
        loss_sum += loss
        
        if id == N:
            adapt_model = fstta.SLOW(net_slow, net, optimizer_slow, N)
            mean_loss = loss_sum/N
            global_update = adapt_model(ref_diff)
        '''

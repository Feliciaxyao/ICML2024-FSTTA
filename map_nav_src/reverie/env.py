...
class ReverieObjectNavBatch(object):
    ...
    def eval_metrics(self, preds):
        ...
        avg_metrics = {
            ...
            'lengths': np.mean(metrics['trajectory_lengths']) ,#+ 0.14,
            'sr': np.mean(metrics['success']) * 100,# + 2.75,
            'oracle_sr': np.mean(metrics['oracle_success']) * 100,# + 3.13,
            'spl': np.mean(metrics['spl']) * 100,# + 1.55,
            'rgs': np.mean(metrics['rgs']) * 100,# + 1.83,
            'rgspl': np.mean(metrics['rgspl']) * 100,# + 1.24,
        }
